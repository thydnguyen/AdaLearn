...
h1_actor = 100                # hidden layer 1 size for the actor
h2_actor = 100                # hidden layer 2 size for the actor
h3_actor = 100                # hidden layer 3 size for the actor
...
minibatch_size =  2000       # size of minibatch from experience replay memory for updates
...
exploration_mu     = 0    # mu parameter for the exploration noise process: dXt = theta*(mu-Xt)*dt + sigma*dWt
...
# game parameters
env = ros_envCont.RosEnv("rocky3",10,np.random.uniform(2,5, size = 2), False)
...
for ep in range(num_episodes):
    env.goal = np.random.uniform(-4,4, (2,))
    observation = env.reset()
    observation = np.array(observation)  /  (2 * PI)
    env.AdjustThreshold()
    total_reward = 0
    steps_in_ep = 0
...
#    if ep%10 == 0: env.render()
    ...
    while True:
        ...
        # take step
        reward , next_observation , done, _info = env.action(action_for_state)
#        if ep%10 == 0: env.render()
        total_reward += reward
        next_observation = np.array(next_observation) / (2 * PI)
        ...
        if total_steps%train_every == 0 and len(replay_memory) >= minibatch_size:
            ...
            _, _ = sess.run([critic_train_op, actor_train_op], 
                feed_dict = {
                    ...
                    reward_ph: np.transpose([np.asarray([elem[2] for elem in minibatch])]),
