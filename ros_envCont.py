# -*- coding: utf-8 -*-
"""
Created on Tue Oct 10 21:54:10 2017
...
class RosEnv(object):
    ...
    def link_states_callback(self, data):
        ...
        self.orient = data.pose[data.name.index('adabot::base_link')].orientation
        quart  = (self.orient.x , self.orient.y, self.orient.z, self.orient.w)
        self.euler = euler_from_quaternion(quart)
        
#        self.pos_xF = data.pose[data.name.index('adabot::front_left_wheel')].position.x
#        self.pos_yF = data.pose[data.name.index('adabot::front_left_wheel')].position.y
#        self.pos_xR = data.pose[data.name.index('adabot::rear_left_wheel')].position.x
#        self.pos_yR = data.pose[data.name.index('adabot::rear_left_wheel')].position.y
        
        self.base_x = data.pose[data.name.index('adabot::base_link')].position.x
        self.base_y = data.pose[data.name.index('adabot::base_link')].position.y
        
#        
#        direct = [self.pos_xF  - self.pos_xR, self.pos_yF - self.pos_yR]

 
        
        #self.direct_path = math.atan2(direct[1], direct[0]) 
        y_path , x_path = [self.goal[1] - self.base_y, self.goal[0] - self.base_x]
        self.goal_path =  math.atan2(y_path, x_path)
        #if record_state:
#            print '\nposition.x: {0}\nsim_time: {1}\n'.format(
...
#                    exit(1)
    ...
    def __init__(self,name, PUB_RATE, goal, source = False):
        ...
        self.euler = [0, 0, 0]
        self.timestep = 0
        self.threshold_dist = np.sqrt(2)
        self.observation_space = np.array(range(1))
        
        rospy.wait_for_service('/gazebo/unpause_physics')
        rospy.Subscriber('clock', Clock, self.clock_callback)
        rospy.Subscriber('gazebo/link_states', LinkStates, self.link_states_callback)
        time.sleep(2)
        self.action_space  = np.array(range(2))
        self.action_space_l = np.array([-.6,-.6])
        self.action_space_h  =  - self.action_space_l
        self.threshold_time = round(self.Distance() * 100)
    ...
    def get_stateVec_raw(self):
        return [self.pos_xF, self.pos_yF, self.euler]
    def get_stateVec(self):
#        return [angle(self.AB, self.CB, self.AC)]
        dp = self.euler[-1]
        gp = self.goal_path
        #return [dp - gp]
        return [dp - gp]
...
#            self.updateRWheel
        #a_time = time.time()
        self.l_wh, self.r_wh = a

#        self.r_wh = self.l_wh * rate
        #self.r_wh = self.l_wh * factor
        #self.r_wh = self.l_wh * factor
        self.updateLWheel()
        self.updateRWheel()
            #time.sleep(10)
...
#        
#        #b_time = time.time()
#        #print("Elapsed", b_time - a_time)
#        #or self.timestep > self.threshold_time
#        #or  self.timestep > self.threshold_time 
#        #or  self.timestep > self.threshold_time 
#        #or  self.timestep > (self.threshold_time * 20) 
        if  new_distance < self.threshold_dist or  self.timestep > (self.threshold_time * 20)   :
            done = True
        if new_distance > 4 * self.threshold_dist:
            done = True
        solve = False if new_distance > self.threshold_dist else True
        self.v = self.r_wh + self.l_wh
        present = self.get_stateVec()
        direction = np.cos(present)
        
        
        #self.reward = 1 if  (direction * self.v)  > 0 else -1 
        self.reward =  direction * self.v -0.8

        #self.reward = direction 
            
        return self.reward[0] , self.get_stateVec(),  done, solve
    ...
    def Distance(self):
        return euclidean([self.base_x, self.base_y], self.goal)
