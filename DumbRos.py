# -*- coding: utf-8 -*-
"""
Created on Tue Oct 17 15:23:53 2017
...
# -*- coding: utf-8 -*-
"""
Created on Tue Oct  3 22:59:30 2017
...
h1_actor = 24                # hidden layer 1 size for the actor
h2_actor = 24                # hidden layer 2 size for the actor
h3_actor = 24                # hidden layer 3 size for the actor
...
minibatch_size = 400    # size of minibatch from experience replay memory for updates
...
exploration_mu     = [0.3, 0.3]    # mu parameter for the exploration noise process: dXt = theta*(mu-Xt)*dt + sigma*dWt
...
# game parameters
env = ros_envCont.RosEnv("rocky3",100,np.random.uniform(2,5, size = 2), False)
...
# placeholders
state_ph = tf.placeholder(dtype=tf.float32, shape=[None,state_dim], name = "obP")
action_ph = tf.placeholder(dtype=tf.float32, shape=[None,action_dim], name ="acP")
reward_ph = tf.placeholder(dtype=tf.float32, shape=[None,1], name = "rPh")
next_state_ph = tf.placeholder(dtype=tf.float32, shape=[None,state_dim], name = "nobPh")
...
def generate_actor_network(s, trainable, reuse):
    hidden = tf.layers.dense(s, h1_actor, activation = None, trainable = trainable, name = 'dense', reuse = reuse)
    hidden_drop = tf.layers.dropout(hidden, rate = dropout_actor, training = trainable & is_training_ph)
    hidden_2 = tf.layers.dense(hidden_drop, h2_actor, activation = None, trainable = trainable, name = 'dense_1', reuse = reuse)
    hidden_drop_2 = tf.layers.dropout(hidden_2, rate = dropout_actor, training = trainable & is_training_ph)
    hidden_3 = tf.layers.dense(hidden_drop_2, h3_actor, activation = None, trainable = trainable, name = 'dense_2', reuse = reuse)
    ...
    actions = env.action_space_l + tf.nn.sigmoid(actions_unscaled)  *(env.action_space_h - env.action_space_l) # bound the actions to the valid range
    return actions
...
def generate_critic_network(s, a, trainable, reuse):
    ...
    hidden = tf.layers.dense(state_action, h1_critic, activation = None, trainable = trainable, name = 'dense', reuse = reuse)
    hidden_drop = tf.layers.dropout(hidden, rate = dropout_critic, training = trainable & is_training_ph)
    hidden_2 = tf.layers.dense(hidden_drop, h2_critic, activation = None, trainable = trainable, name = 'dense_1', reuse = reuse)
    hidden_drop_2 = tf.layers.dropout(hidden_2, rate = dropout_critic, training = trainable & is_training_ph)
    hidden_3 = tf.layers.dense(hidden_drop_2, h3_critic, activation = None, trainable = trainable, name = 'dense_2', reuse = reuse)
...
for ep in range(num_episodes):
    env.goal = np.random.uniform(-7,7, (2,))
    ...
    noise_scale = (initial_noise_scale * noise_decay**ep) * (env.action_space_h  - env.action_space_l)
...
#    if ep%10 == 0: env.render()
    ...
    while True:
        #action = [0.9, -.9]
        #action = ""
        #print("Direct ",env.direct_path)
#        print("Goal ",env.euler[-1])
#        print("Actual goal", env.goal_path)
        observation = env.get_stateVec()
        #print(observation)
        #print("REAL GOAL", env.goal)
        if np.abs(observation[0]) < 0.002:
            print(observation)
            action = [1,1] #Go
        ...
        action = np.array(action) * 0.4

             #Spin
        # choose action based on deterministic policy
#        action_for_state, = sess.run(actions, 
#            feed_dict = {state_ph: [observation], is_training_ph: False})
#
...
#        action_for_state += noise_scale*noise_process
        ...
        # take step
        reward , next_observation , done, _info = env.action(action)
#        if ep%10 == 0: env.render()
        total_reward += reward
...
#        print("Reward", reward)
#        print("Goal", env.goal)
    print('Episode %2i, Reward: %7.3f, Steps: %i, Final noise scale: %7.3f'%(ep,total_reward,steps_in_ep, noise_scale[0]))
